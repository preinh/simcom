...
#from matlib import matprint, mataugprint, matvec, matcopy
...
def gausselim(A, B, pivoting = 0, ztol = 1.0e-8, debug = False):
    ...
    for pivot in range( size - 1) :
        ...
        if debug: print (pivot, absm, exchrow, exchcol)
        ...
        #Exchange columns ?
        if pivot != exchcol:
            if debug: print "--> exchanging columns", pivot, exchcol
...
if __name__ == "__main__":
    ...
    B = array([28., -40., 33.])
    Bcopy = B.copy()
    code, R, C, A, X, B = gausselim(A,B, pivoting=0, debug =True)
    print "code=", code
    print "X= ", X
    print "A X = ", dot(Acopy, X)
